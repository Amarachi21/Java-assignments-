...
// Track old  for value ramping
float oldLow = Low;
float oldMid = Mid;
float oldHi = Hi;

// Rate at which the score Reduces
float scoreReduceRate = 25;
...
void draw()
{
  ...
  if (oldLow > Low) {
    Low = oldLow - scoreReduceRate;
  }
  
  if (oldMid > Mid) {
    Mid = oldMid - scoreReduceRate;
  }
  
  if (oldHi > Hi) {
    Hi = oldHi - scoreReduceRate;
  }
  
  // # Create a "global" score that calculates the percieved intensity of all zones
  float scoreGlobal = 0.66*Low + 0.8*Mid + 1*Hi;
  
  // Set subtle background color based on intensity
  background(Low/100, Mid/100, Hi/100);
  ...
  for(int i = 0; i < nbSpheres; i++)
  {
    ...
    spheres[i].display(Low, Mid, Hi, band, scoreGlobal);
  }
...
// Distance between each line point, negative because on the z dimension
  ...
  for(int i = 1; i < fft.specSize(); i++)
  {
    ...
    stroke(100+Low, 100+Mid, 100+Hi, 255-i);
  ...
  for(int i = 0; i < nbMurs; i++)
  {
    ...
    murs[i].display(Low, Mid, Hi, intensity, scoreGlobal);
  }
}
